...
class AppSettingsService extends GetxController {
  ...
  void onInit() {
    ...
    //漫画
    comicReaderDirection.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderDirection,
      0,
    );
    comicReaderFullScreen.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderFullScreen,
      true,
    );
    comicReaderShowStatus.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderShowStatus,
      true,
    );
    comicReaderShowStatus.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderShowStatus,
    ...
    comicReaderShowViewPoint.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderShowViewPoint,
      true,
    );
    comicReaderLeftHandMode.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderLeftHandMode,
      false,
    );
    comicReaderHD.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderHD,
      false,
    );
    comicReaderPageAnimation.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderPageAnimation,
      true,
    );
    comicReaderOldViewPoint.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicReaderOldViewPoint,
      false,
    );
    //小说
    novelReaderDirection.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderDirection,
      0,
    );
    novelReaderFontSize.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderFontSize,
      16,
    );
    novelReaderLineSpacing.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderLineSpacing,
      1.5,
    );
    novelReaderTheme.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderTheme,
      0,
    );
    novelReaderFullScreen.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderFullScreen,
      true,
    );
    novelReaderShowStatus.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderShowStatus,
      true,
    );
    novelReaderLeftHandMode.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderLeftHandMode,
      false,
    );
    novelReaderPageAnimation.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNovelReaderPageAnimation,
      true,
    );
    //下载
    downloadAllowCellular.value = LocalStorageService.instance.getValue(
      LocalStorageService.kDownloadAllowCellular,
      true,
    );
    downloadComicTaskCount.value = LocalStorageService.instance.getValue(
      LocalStorageService.kDownloadComicTaskCount,
      5,
    );
    downloadNovelTaskCount.value = LocalStorageService.instance.getValue(
      LocalStorageService.kDownloadNovelTaskCount,
      5,
    );
    //搜索API
    comicSearchUseWebApi.value = LocalStorageService.instance.getValue(
      LocalStorageService.kComicSearchUseWebApi,
      false,
    );
    //字体大小
    useSystemFontSize.value = LocalStorageService.instance.getValue(
      LocalStorageService.kUseSystemFontSize,
      false,
    );
    //新闻字体
    newsFontSize.value = LocalStorageService.instance.getValue(
      LocalStorageService.kNewsFontSize,
      15,
    );
    //自动添加神隐漫画至收藏夹
    collectHideComic.value = LocalStorageService.instance.getValue(
      LocalStorageService.kCollectHideComic,
      false,
    );
    super.onInit();
  }
    ...
    Get.dialog(
      RadioGroup<int>(
        ...
        child: const SimpleDialog(
          title: Text("设置主题"),
          children: [
            RadioListTile<int>(title: Text("跟随系统"), value: 0),
            RadioListTile<int>(title: Text("浅色模式"), value: 1),
            RadioListTile<int>(title: Text("深色模式"), value: 2),
          ],
        ),
      ),
    );
